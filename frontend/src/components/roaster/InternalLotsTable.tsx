...
import { Package, Star, ChevronLeft, ChevronRight, PlusCircle, ClipboardList, X } from 'lucide-react';
...
const InternalLotsTable: React.FC<InternalLotsTableProps> = ({ lots, onLogRoast, currentPage = 1, totalPages = 1, onPageChange, hideHeader = false }) => {
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm text-gray-500 font-medium">No withdrawal lots available</p>
                    <p className="text-xs text-gray-400 mt-1">Withdraw green bean stock to start roasting</p>
                  </div>
                </td>
              </tr>
            ) : (
          ...
          <div className="px-4 py-3 space-y-4">
            ...
            <div className="border-t border-gray-100 pt-2 space-y-4">
              <div className="flex justify-between items-center gap-4">
                ...
                <span className="text-xs font-mono font-normal text-gray-800">{activeLot.greenBeanDisplayId || '—'}</span>
              </div>
              <div className="flex justify-between items-center gap-4">
                <span className="text-xs text-gray-500">Withdrawal Type</span>
                <span className="text-xs font-normal text-indigo-700 bg-indigo-50 px-1.5 py-0.5 rounded">{activeLot.withdrawalType || '—'}</span>
              </div>
            </div>
          </div>
        </div>
