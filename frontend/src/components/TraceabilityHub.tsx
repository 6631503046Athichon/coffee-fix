    ...
    try {
      return data.greenBeanLots
        .map((gbl) => {
          ...
          if (scoreInfo) {
            try {
              const session = data.cuppingSessions?.find(
                (s) => s?.id === scoreInfo.sessionId,
              );
              const sample = session?.samples?.find(
                (s) => s?.greenBeanLotId === gbl.id,
              );
              if (
                session &&
                sample &&
                session.finalResults &&
                session.finalResults[sample.id]
              ) {
                finalScore =
                  session.finalResults[sample.id].totalScore.toFixed(2);
              } else if (scoreInfo.score) {
                // Fallback to score on GBL if final results not compiled
          ...
          return {
            ...
            finalScore,
            processorScore: gbl.processorScore,
          };
        })
