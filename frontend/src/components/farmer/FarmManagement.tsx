...
import { PlusCircle, Sprout, MapPin, Leaf, Coffee, Search, ShieldCheck, Layers3, Compass, X, MoreVertical, Edit3, Archive, RotateCcw, Save, Microscope, FlaskConical, Trash2, CheckCircle } from 'lucide-react';
...
const formatDateDisplay = (isoDate?: string) => {
	...
	return Number.isNaN(date.getTime()) ? isoDate : date.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' });
};
...
const FarmManagement: React.FC = () => {
	...
	const [selectedSoilFarm, setSelectedSoilFarm] = useState<Farm | null>(null);
	...
	const selectedFarmAnalyses = useMemo(() => {
		if (!selectedSoilFarm) return [];
		return data.soilAnalyses
			.filter(analysis => analysis.farmId === selectedSoilFarm.id)
			.sort((a, b) => new Date(b.testDate).getTime() - new Date(a.testDate).getTime());
	}, [data.soilAnalyses, selectedSoilFarm]);
	...
	const handleOpenSoilModal = (farm: Farm) => {
		setSelectedSoilFarm(farm);
	...
	const handleCloseSoilModal = () => {
		setIsSoilModalOpen(false);
		setSelectedSoilFarm(null);
		setEditingSoilId(null);
		setSoilFormError(null);
		setSoilToast(null);
	...
	const handleSoilCancelEdit = () => {
		setEditingSoilId(null);
		setSoilFormError(null);
		setSoilForm(createEmptySoilForm({ farmPlotLocation: selectedSoilFarm?.location ?? '' }));
	};
	...
	const handleSoilDelete = (analysisId: string) => {
		if (!confirm('ยืนยันการลบผลวิเคราะห์ดินนี้?')) return;
		setData(prev => ({
			...prev,
	...
	return (
		<div className="space-y-6">
			...
			{showInitialEmptyState || showSearchEmptyState ? (
				...
				<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
					{filteredFarms.map(farm => (
						<div
							key={farm.id}
							className={`bg-white border rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow ${farm.archived ? 'border-amber-200 bg-amber-50/40' : 'border-gray-200'} cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-300`}
							onClick={() => handleOpenSoilModal(farm)}
							onKeyDown={event => {
								if (event.key === 'Enter' || event.key === ' ') {
									event.preventDefault();
									handleOpenSoilModal(farm);
								}
							}}
							tabIndex={0}
							role="button"
							aria-label={`เปิดข้อมูลดินของฟาร์ม ${farm.name ?? farm.location}`}
						>
			...
			<Modal
				...
				onClose={handleCloseSoilModal}
				title={selectedSoilFarm ? `ข้อมูลดิน: ${selectedSoilFarm.name ?? selectedSoilFarm.location}` : 'ข้อมูลดินฟาร์ม'}
				maxWidth="5xl"
			>
				{selectedSoilFarm ? (
					<div className="space-y-6">
						<div className="flex items-start gap-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
							...
							<div>
								<p className="text-sm font-semibold text-emerald-700">{selectedSoilFarm.id}</p>
								<h2 className="text-xl font-bold text-gray-900">{selectedSoilFarm.name ?? selectedSoilFarm.location}</h2>
								<p className="text-sm text-gray-600">{selectedSoilFarm.location}</p>
								<p className="text-sm text-gray-600">เจ้าของ: {selectedSoilFarm.ownerName ?? selectedSoilFarm.farmerName}</p>
								<p className="text-xs text-gray-500 mt-1">คลิกการ์ดฟาร์มเพื่อเปิดแบบฟอร์มข้อมูลดิน เพิ่ม แก้ไข และบันทึกประวัติได้ทันที</p>
							</div>
						</div>
