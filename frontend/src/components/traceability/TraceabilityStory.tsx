...
import { Coffee, Thermometer, Droplets, Droplet, QrCode, Printer, Flame, Copy, Check, MapPin } from 'lucide-react';
import { PublicTraceData } from '@/services/lots/greenBeanLotService';
...
interface TraceabilityStoryProps {
  data: PublicTraceData;
  // The page's public address. null hides the QR / share section: a lot that
  // has not been published yet has no address customers can open.
  shareUrl: string | null;
}

// The farm-to-cup story of one green bean lot. Customers see it on the public
// /trace page; staff see the same card as a preview on /traceability/:lotId.
const TraceabilityStory: React.FC<TraceabilityStoryProps> = ({ data, shareUrl }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in overflow-hidden rounded-3xl bg-white shadow-xl my-8">
      ...
      {/* Section 2: QR Code Share — only once the lot has a public address */}
      {shareUrl && (
...
export default TraceabilityStory;
