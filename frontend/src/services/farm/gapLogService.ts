...
// Raw backend payload for a GAP log. The transformer in `../utils/transformers`
// (consumed via `transformGAPLogFromBackend`) is responsible for shaping it into
// the strict frontend `GAPLogEntry` type.
type BackendGAPLog = Record<string, unknown>;

interface UpdateGAPLogPayload {
  farmId?: string | null;
...
const findActivityTypeId = async (activityTypeName: string): Promise<string> => {
  const response = await api.get<{ activityTypes: BackendActivityType[] }>('/activity-types');
  const activityType = response.activityTypes.find(at => at.name === activityTypeName);
...
export const getAllGAPLogs = async (
  ...
  try {
    ...
    const response = await api.get<{ gapLogs: BackendGAPLog[] }>(
      '/gap-logs',
...
export const addGAPLog = async (logData: Partial<GAPLogEntry>): Promise<GAPLogEntry> => {
  try {
    ...
    const response = await api.post<{ gapLog: BackendGAPLog; message: string }>('/gap-logs', {
...
export const updateGAPLog = async (
  ...
  try {
    const updatePayload: UpdateGAPLogPayload = {
    ...
    const response = await api.put<{ gapLog: BackendGAPLog }>(`/gap-logs/${logId}`, updatePayload);
    return transformGAPLogFromBackend(response.gapLog);
  } catch (error) {
