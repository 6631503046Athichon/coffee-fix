...
} from './services/weatherAutoFetchService'
...
const getDashboardPathByRole = (roles: UserRole[]): string => {
  ...
  if (normalizedRoles.includes(UserRole.Roaster.toLowerCase())) return '/roaster'
  if (normalizedRoles.includes(UserRole.Processor.toLowerCase())) return '/processor'
  if (
    normalizedRoles.includes(UserRole.Cupper.toLowerCase()) ||
    normalizedRoles.includes(UserRole.HeadJudge.toLowerCase())
  )
    return '/cupping'
  if (
    normalizedRoles.includes(UserRole.Farmer.toLowerCase()) ||
...
const ProtectedRoutes: React.FC = () => {
  ...
  const mergeArrays = useCallback(
    <T extends { id: string }>(backendData: T[], mockData: T[]): T[] => {
  ...
  const loadDataFromBackend = useCallback(async () => {
    try {
      ...
      const storedProcessingBatches = phase2.processingBatches.map(
      ...
      setData((prev) => ({
        ...
        pricingHistory:
          storedPricingHistory.length > 0 ? storedPricingHistory : prev.pricingHistory,
        soilAnalyses: storedSoilAnalyses.length > 0 ? storedSoilAnalyses : prev.soilAnalyses,
        weatherRecords:
          storedWeatherRecords.length > 0 ? storedWeatherRecords : prev.weatherRecords,
        ...
        roasterInventory:
          storedRoasterInventory.length > 0 ? storedRoasterInventory : prev.roasterInventory,
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (
        e.key &&
        (e.key === 'coffee_lab_process_types' || e.key === 'coffee_lab_activity_types')
      ) {
        debouncedRefresh()
      }
    }
  ...
  const contextValue = useMemo(
    () => ({ data, setData, refreshData, setIsEditing, isEditing }),
    [data, setData, setIsEditing, isEditing],
  )
  ...
  const navItems = useMemo(() => {
    ...
    if (currentUser) {
      const judgeSessions = data.cuppingSessions.filter(
        (s) =>
      ...
      const activeSession = judgeSessions.find(
        (s) => s.status === 'Adjudication' || s.status === 'Scoring',
      )
      ...
      if (activeSession) {
        ...
        const sortedSessions = [...judgeSessions].sort(
      ...
      const firstCompetition = data.cuppingSessions.find(
        (s) => s.type === CuppingSessionType.Competition,
      )
      if (firstCompetition) {
    ...
    return [
      ...
      },
      {
        ...
        href: '/customers',
        icon: Users,
        roles: [UserRole.Admin, UserRole.Roaster],
        section: 'roaster',
      },
      {
        name: 'Quality Insights',
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden w-full lg:w-auto">
          ...
          <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-3 sm:p-4 md:p-6 lg:p-8 pt-16 lg:pt-4">
            <Routes>
              ...
              <Route
                path="/cupping/:id"
                element={<CuppingSessionDetail currentUser={currentUser!} />}
              />
              ...
              <Route
                ...
                element={
                  <ProtectedRoute
                    allowedRoles={[UserRole.Processor, UserRole.Roaster, UserRole.Admin]}
                  >
                    <QualityInsights />
                  </ProtectedRoute>
                }
